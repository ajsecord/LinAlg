...
namespace {
    ...
    template <typename T>
    struct LapackHelper4 {
        static void eigs(const Mat4<T>& A, 
                         Vec4<T>& values, Mat4<T>& vectors) {
...
#   include "Lapack.h"
    ...
    template <>
    struct LapackHelper4<float> {
        static void eigs(const Mat4<float>& A, 
                         Vec4<float>& values, Mat4<float>& vectors) {
            ...
            int lda   = 4;            // Leading dimension
            int lwork = (getBlockSize(4)+2)*4; // Size of additional work space
            int info;
    ...
    template <>
    struct LapackHelper4<double> {
        static void eigs(const Mat4<double>& A, 
                         Vec4<double>& values, Mat4<double>& vectors) {
            char jobz = 'V';          // Compute both eigenvalues and eigenvectors
            char uplo = 'L';          // Lower triangle of A (at least) is stored
            int n     = 4;            // Order of the matrix
            int lda   = 4;            // Leading dimension
    ...
    struct MatFloatingPointHelper {
        static void eigs(const Mat4<T>& A, Vec4<T>& values, Mat4<T>& vectors) {
            LapackHelper4<T>::eigs(A, values, vectors);
        }
    };
...
template <typename T>
Vec4<T> Mat4<T>::getCol(const unsigned c) const {
    const unsigned offset = c * 4;
    Vec4<T> v;
    for (unsigned i = 0; i < 4; ++i)
        v(i) = m_data[offset + i];
    return v;
...
template <typename T>
void Mat4<T>::setCol(const unsigned c, const Vec4<T>& v) {
    const unsigned offset = c * 4;
    for (unsigned i = 0; i < 4; ++i)
        m_data[offset + i] = v(i);
}
...
template <typename T>
Vec4<T> Mat4<T>::getRow(const unsigned r) const {
    Vec4<T> v;
    for (unsigned i = 0; i < 4; ++i)
        v(i) = m_data[i * 4 + r];
    return v;
...
template <typename T>
void Mat4<T>::setRow(const unsigned r, const Vec4<T>& v) {
    for (unsigned i = 0; i < 4; ++i)
        m_data[i * 4 + r] = v(i);
}

template <typename T>
Mat4<T> Mat4<T>::trans() const {
    Mat4<T> w;
    for (unsigned i = 0; i < 4; ++i) {
        for (unsigned j = 0; j < 4; ++j) {
            w.m_data[j * 4 + i] = m_data[i * 4 + j];
        }
    }
...
template <typename T>
const Mat4<T>& Mat4<T>::ident() {
    static bool needInit = true;
    static Mat4<T> identMat;
    if (needInit) {
        for (unsigned j = 0; j < 4; ++j)
            for (unsigned i = 0; i < 4; ++i)
                identMat(i,j) = (i == j);
...
template <typename T>
bool Mat4<T>::equal(const Mat4<T>& other, const T eps) const {
...
template <typename T>
Mat4<T> Mat4<T>::inverse(const Vec4<T>& values, const Mat4<T>& vectors) {
...
template <typename T>
void Mat4<T>::eigs(Vec4<T>& values, Mat4<T>& vectors) const {
#if 4 == 2
...
template <typename T>
std::string Mat4<T>::description() {
    static std::string d;
    if (d.empty()) {
        std::ostringstream o;
        o << 4 << "D " << type_description<T>() << " matrix";
        d = o.str();
    }
...
template class Mat4<type>; \
template std::ostream& operator<<(std::ostream& o, const Mat4<type>& v); \
template std::istream& operator>>(std::istream& i, Mat4<type>& v); \
template Mat4<type> operator*(const Mat4<type>& u, const Mat4<type>& v); \
template Vec4<type> operator*(const Mat4<type>& m, const Vec4<type>& v); \
template Mat4<type> min(const Mat4<type>& u, const Mat4<type>& v); \
template Mat4<type> max(const Mat4<type>& u, const Mat4<type>& v);
