...
template <typename T>
const Vec2<T> Vec2<T>::unit(const unsigned index) {
    assert(index < 2);
    Vec2<T> v = zero();
    v.m_data[index] = 1;
    return v;
...
template <typename T>
bool Vec2<T>::equal(const Vec2<T>& v, const T eps) const {
    for (unsigned i = 0; i < 2; ++i)
...
template <typename T>
std::string Vec2<T>::description() {
    static std::string d;
    if (d.empty()) {
        std::ostringstream o;
        o << 2 << "D " << type_description<T>() << " vector";
        d = o.str();
    }
...
template class Vec2<T>; \
template std::ostream& operator<<(std::ostream& o, const Vec2<T>& v); \
template std::istream& operator>>(std::istream& i, Vec2<T>& v);
